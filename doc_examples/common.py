...
def printAsShortcodes(inputs, results, outf=sys.stdout):
    tracks = inputs + results
    labels = []
    shortcodes = []
    for tIndex in range(len(tracks)):
        t = tracks[tIndex]
        n = 'i%u' % tIndex
        if tIndex >= len(inputs):
          n = 'r%u' % (tIndex - len(inputs))
        if hasattr(t, 'name'):
          n = t.name
        labels.append(n)
...
def doIt(args, input_, results, label, useNarrow=True, splitResults=False, zeroAndOneAreSpecial=3):
    ...
    if args.mode == MODE_WAVEDROM:
        print(resultAsWavedrom(input_, results, label, useNarrow, splitResults, zeroAndOneAreSpecial), file=args.outfile)
...
""".format(args.progname, args.progname), file=args.outfile)
        ...
        wavedrom = resultAsWavedrom(input_, results, None, useNarrow, splitResults, zeroAndOneAreSpecial)
